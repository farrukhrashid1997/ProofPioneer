...
def worker_process(task_queue: Queue, output_queue: Queue, api_key: str, prompt_template: str):
    ...
    while True: 
        ...
        # Prepare the prompt by replacing placeholders
            ...
            prompt = prompt_template.replace("[Insert the claim here]", record['claim'])
            prompt = prompt.replace("[Insert the reasoning here]", record['text'])
            prompt = prompt.replace("[Insert the label here]", record['label'])
            ...
            record['justification'] = resp.get("Summary", None)
            record['claim_author'] = resp.get("Claim Author", None)
            record['claim_date'] = resp.get("Claim Date", None)
...
def main():
    ...
    output_file = "snopes_results_with_justification.csv"
