...
if __name__ == "__main__":
    example_claims_df = pd.read_csv("./claim_datasets/x-fact/all_languages/train-00000-of-00001.csv")  
    ...
    gemini_flash_api = GeminiAPI(secrets_file="./secrets/gemini_keys.json", model_name="gemini-1.5-flash-latest")
    ...
    existing = {}
    for _, claim_series in tqdm(claims_to_process.iterrows(), total=len(claims_to_process), desc="Processing Claims"):
        claim = claim_series["claim"]
        ...
        sort_date = extract_and_format_date(claim_series["claimDate"], default_date=min_date)
        search_strings = []
        search_types = []
